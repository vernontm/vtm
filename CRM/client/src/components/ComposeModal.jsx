...
export default function ComposeModal({ onClose, onComplete }) {
  ...
  function genBtnStyle(source, accentColor, borderColor) {
    ...
    return {
      ...
      background: busy ? '#111110' : accentColor + '18',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: '18px 20px 14px', borderBottom: '1px solid #252523',
          position: 'sticky', top: 0, background: '#161614', zIndex: 2,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <PenLine size={16} color="#c8f135" />
            <span style={{ fontSize: 15, fontWeight: 700, color: '#e8e6df' }}>Compose Email</span>
          </div>
        ...
        <div style={{ padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            {selectedLead ? (
              <div style={{
                ...
                background: '#111328', border: '1px solid #252523',
              }}>
                <span style={{ fontWeight: 600, color: '#e8e6df', fontSize: 13, flex: 1 }}>{selectedLead.full_name}</span>
              ...
              <div style={{ position: 'relative' }} ref={toRef}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '8px 12px 8px 30px', borderRadius: 6, fontSize: 13, color: '#e8e6df', background: '#111328', border: '1px solid #252523', outline: 'none', boxSizing: 'border-box' }}
                  />
                </div>
                {showSuggestions && suggestions.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 9999, marginTop: 2, background: '#161614', border: '1px solid #252523', borderRadius: 6, boxShadow: '0 4px 20px rgba(0,0,0,0.5)', overflow: 'auto', maxHeight: 240 }}>
                    {suggestions.map(l => (
                      ...
                      >
                        <span style={{ fontWeight: 600, color: '#e8e6df', flex: 1 }}>{l.full_name}</span>
                    ...
                    {toInput.includes('@') && (
                      <div style={{ padding: '8px 12px', fontSize: 11, color: '#4a4845', borderTop: '1px solid #252523', fontStyle: 'italic' }}>
                        Or press Tab / click outside to send to <strong style={{ color: '#7a7870' }}>{toInput}</strong> directly
                      </div>
                    )}
                  </div>
                )}
                {!showSuggestions && toInput.includes('@') && toInput.includes('.') && (
                  <div style={{ marginTop: 6, fontSize: 11, color: '#4a4845' }}>
                    Sending to: <span style={{ color: '#c8f135', fontWeight: 600 }}>{toInput}</span> (custom address — not linked to a lead)
                  </div>
                )}
              </div>
            )}
          ...
          {selectedLead && (
            <div style={{ background: '#111328', border: '1px solid #252523', borderRadius: 8, padding: '12px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#4a4845' }}>Gmail History</span>
                <button
                  onClick={handleSync}
                  disabled={syncing}
                  style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: syncing ? 'not-allowed' : 'pointer', background: '#111110', border: '1px solid #252523', color: '#c8f135', opacity: syncing ? 0.7 : 1 }}
                >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 10 }}>
                  ...
                  {syncResult.hasSent ? (
                    <div style={{ background: '#141e36', border: '1px solid #1e3a5f', borderRadius: 6, padding: '10px 12px' }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: '#c8f135', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
                        📤 Last Sent to Lead
                      </div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#7a7870', marginBottom: 2 }}>
                      ...
                      <div style={{ borderTop: '1px solid #1e3a5f', paddingTop: 8, marginTop: 4 }}>
                        <textarea
                          ...
                          rows={2}
                          style={{ width: '100%', padding: '7px 10px', borderRadius: 5, fontSize: 11, color: '#7a7870', background: '#0f1830', border: '1px solid #1e3a5f', outline: 'none', resize: 'vertical', lineHeight: 1.5, fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: 8 }}
                        />
                        <button
                          onClick={() => handleCreateEmail('sent')}
                          disabled={!!generatingFor}
                          style={genBtnStyle('sent', '#c8f135', '#1e3a5f')}
                        >
                          {generatingFor === 'sent'
                  ...
                  {syncResult.hasReply ? (
                    <div style={{ background: '#142018', border: '1px solid #1e4028', borderRadius: 6, padding: '10px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#c8f135', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                          💬 Their Last Reply
                        </div>
                      ...
                      <div style={{ borderTop: '1px solid #1e4028', paddingTop: 8, marginTop: 4 }}>
                        <textarea
                          ...
                          rows={2}
                          style={{ width: '100%', padding: '7px 10px', borderRadius: 5, fontSize: 11, color: '#7a7870', background: '#0f1a12', border: '1px solid #1e4028', outline: 'none', resize: 'vertical', lineHeight: 1.5, fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: 8 }}
                        />
                        <button
                          onClick={() => handleCreateEmail('reply')}
                          disabled={!!generatingFor}
                          style={genBtnStyle('reply', '#c8f135', '#1e4028')}
                        >
                          {generatingFor === 'reply'
                            ? <Loader size={11} style={{ animation: 'spin 0.7s linear infinite' }} />
                            : <Zap size={11} />}
          ...
          {generatedItemId && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: 6, background: '#0f2318', border: '1px solid #1e4028', fontSize: 11, color: '#c8f135' }}>
              <Zap size={11} />
          ...
          <div>
            ...
            <select
              ...
              style={{ width: '100%', padding: '8px 10px', borderRadius: 6, fontSize: 13, color: '#7a7870', background: '#111328', border: '1px solid #252523', cursor: 'pointer', outline: 'none' }}
            >
          ...
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: '8px 12px', borderRadius: 6, fontSize: 13, color: '#e8e6df', background: '#111328', border: '1px solid #252523', outline: 'none', boxSizing: 'border-box' }}
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={10}
              style={{ width: '100%', padding: '12px 14px', borderRadius: 6, fontSize: 13, color: '#7a7870', background: '#111328', border: '1px solid #252523', outline: 'none', resize: 'vertical', lineHeight: 1.6, fontFamily: 'inherit', boxSizing: 'border-box' }}
            />
          </div>

          {/* ── Global context / notes ────────────────────────────────────────── */}
          <div style={{ borderTop: '1px solid #252523', paddingTop: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            ...
            <textarea
              ...
              rows={3}
              style={{ width: '100%', padding: '10px 12px', borderRadius: 6, fontSize: 12, color: '#7a7870', background: '#111328', border: '1px solid #252523', outline: 'none', resize: 'vertical', lineHeight: 1.5, fontFamily: 'inherit', boxSizing: 'border-box' }}
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button
              onClick={onClose}
              style={{ padding: '8px 16px', borderRadius: 6, fontSize: 13, cursor: 'pointer', background: 'none', border: '1px solid #252523', color: '#4a4845' }}
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 20px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', background: '#c8f135', border: 'none', color: '#0a0a08', opacity: saving ? 0.7 : 1 }}
            >
              {saving
