    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:'var(--bg)', fontFamily:'var(--font-display)' }}>
      ...
      <div style={{ padding:'20px 28px 16px', background:'var(--surface)', borderBottom:'1px solid var(--border)' }}>
        ...
        <div className="grid-4" style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12 }}>
          <div style={{ padding:'14px 18px', background:'var(--surface-2)', borderRadius:10, border:'1px solid var(--border)' }}>
            <div style={{ fontSize:10, color:'var(--muted)', fontWeight:600, textTransform:'uppercase', marginBottom:4 }}>Active</div>
            <div style={{ fontSize:22, fontWeight:700, color:'var(--text)' }}>{activeSubs.length}</div>
          </div>
          <div style={{ flex:1, padding:'14px 18px', background:'var(--surface-2)', borderRadius:10, border:'1px solid var(--border)' }}>
            <div style={{ fontSize:10, color:'var(--muted)', fontWeight:600, textTransform:'uppercase', marginBottom:4 }}>Monthly Cost</div>
            <div className="private-value" style={{ fontSize:22, fontWeight:700, color:'var(--orange)' }}>${monthlyTotal.toFixed(2)}</div>
          </div>
          <div style={{ flex:1, padding:'14px 18px', background:'var(--surface-2)', borderRadius:10, border:'1px solid var(--border)' }}>
            <div style={{ fontSize:10, color:'var(--muted)', fontWeight:600, textTransform:'uppercase', marginBottom:4 }}>Yearly Cost</div>
            <div className="private-value" style={{ fontSize:22, fontWeight:700, color:'var(--text)' }}>${yearlyTotal.toFixed(2)}</div>
          </div>
          <div style={{ flex:1, padding:'14px 18px', background:'var(--surface-2)', borderRadius:10, border:'1px solid var(--border)' }}>
            <div style={{ fontSize:10, color:'var(--muted)', fontWeight:600, textTransform:'uppercase', marginBottom:4 }}>Total Subs</div>
            <div style={{ fontSize:22, fontWeight:700, color:'var(--text)' }}>{subs.length}</div>
          </div>
        </div>
      </div>
      ...
      {scanResult && (
        <div style={{ padding:'10px 28px', background:scanResult.subscriptions?.length ? 'rgba(34,197,94,0.06)' : 'rgba(74,108,247,0.04)', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:8 }}>
          ...
          <button onClick={() => setScanResult(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--muted)', display:'flex' }}><X size={14} /></button>
        </div>
      )}
      ...
      <div style={{ padding:'10px 24px', display:'flex', alignItems:'center', gap:10, background:'var(--surface)', borderBottom:'1px solid var(--border)' }}>
        ...
        <span style={{ marginLeft:'auto', fontSize:12, color:'var(--muted)' }}>{filtered.length} subscription{filtered.length!==1?'s':''}</span>
      </div>
      ...
      <div style={{ flex:1, overflow:'auto', padding:'0 28px 28px' }}>
        {loading ? (
          ...
          <div style={{ textAlign:'center', padding:80 }}>
            <CreditCard size={40} style={{ color:'var(--surface-3)', margin:'0 auto 12px' }} />
            <div style={{ color:'var(--muted)', fontSize:14, fontWeight:600 }}>No subscriptions</div>
            <div style={{ color:'var(--muted)', fontSize:12, marginTop:4 }}>Click "Scan Gmail" to auto-detect or add manually</div>
          </div>
        ) : (
