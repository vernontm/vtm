...
export default function MeetingDetail() {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 400, background: '#0a0a08', color: '#4a4845', gap: 12 }}>
  ...
  if (error) {
    return (
      <div style={{ padding: 32, background: '#0a0a08', minHeight: '100%' }}>
        <button
          onClick={() => navigate('/meetings')}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4a4845', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, marginBottom: 20, padding: 0 }}
        >
  ...
  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <div style={{ padding: 24, minHeight: '100%', background: '#0a0a08' }}>

      {/* ── Back + Header ─────────────────────────────────────────────────── */}
      <div style={{ marginBottom: 20 }}>
        <button
          onClick={() => navigate('/meetings')}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4a4845', display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, marginBottom: 14, padding: 0 }}
        >
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <h1 style={{ fontSize: 21, fontWeight: 800, color: '#e8e6df', margin: 0 }}>{meeting?.title}</h1>
              <StatusBadge status={meeting?.status} />
            </div>
            <div style={{ fontSize: 12, color: '#4a4845', marginTop: 5, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{
            ...
            {activeTab === 'overview' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {/* Recording section */}
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#4a4845', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Video size={12} /> Recording
                  </div>
                  {recordingUrl ? (
                    <div style={{ background: '#111328', border: '1px solid #252523', borderRadius: 10, overflow: 'hidden' }}>
                      <iframe
                        src={recordingUrl}
                      ...
                      />
                      {webViewUrl && (
                        <div style={{ padding: '8px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #252523' }}>
                          <span style={{ fontSize: 11, color: '#4a4845' }}>Recording from Google Drive</span>
                          <a
                            href={webViewUrl}
                            target="_blank"
                            rel="noreferrer"
                    ...
                    <div style={{ background: '#111110', border: '1px solid #252523', borderRadius: 10, padding: '28px 20px', textAlign: 'center' }}>
                      <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#252523', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                        <Video size={22} color="#4a4845" />
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#4a4845', marginBottom: 6 }}>No Recording Found</div>
                      <div style={{ fontSize: 11, color: '#252523', marginBottom: 14 }}>
                ...
                <div>
                  ...
                  {summarizing ? (
                    <div style={{ background: '#111110', border: '1px solid #784bd140', borderRadius: 10, padding: 28, textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 11, color: '#4a4845', marginTop: 5 }}>This may take a few seconds</div>
                    </div>
                  ) : summary ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>

                      {/* Summary title + general summary */}
                      <div style={{ background: '#111110', border: '1px solid #784bd130', borderRadius: 10, padding: '16px 18px' }}>
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#e8e6df', marginBottom: 10 }}>
                          {summary.summary_title || meeting?.title}
                        </div>
                        <div style={{ fontSize: 13, color: '#7a7870', lineHeight: 1.7 }}>
                      ...
                      {summary.topics?.length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: '#4a4845', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10 }}>
                            Topics Discussed
                          </div>
                          {summary.topics.map((topic, i) => (
                            <div key={i} style={{ background: '#1a1d2e', border: '1px solid #252523', borderRadius: 8, padding: '12px 14px', marginBottom: 8 }}>
                              <div style={{ fontSize: 13, fontWeight: 700, color: '#e8e6df', marginBottom: 7 }}>{topic.title}</div>
                              {topic.bullets?.map((b, bi) => (
                                <div key={bi} style={{ fontSize: 12, color: '#7a7870', display: 'flex', gap: 8, marginBottom: 4, alignItems: 'flex-start' }}>
                      ...
                      {summary.follow_up_suggested && (
                        <div style={{ background: '#fdab3d10', border: '1px solid #fdab3d30', borderRadius: 8, padding: '10px 14px', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                          ...
                          <span style={{ fontSize: 12, color: '#7a7870' }}>{summary.follow_up_suggested}</span>
                        </div>
                      )}

                      {/* Meta footer */}
                      <div style={{ fontSize: 10, color: '#252523', textAlign: 'right' }}>
                        Generated {summary.generated_at ? new Date(summary.generated_at).toLocaleDateString() : ''} · {summary.model_used || ''}
                      </div>
                    </div>
                  ) : (
                    <div style={{ background: '#111110', border: '1px solid #784bd130', borderRadius: 10, padding: '30px 20px', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 12, color: '#4a4845', marginBottom: 16, lineHeight: 1.65 }}>
                ...
                {meeting?.participants?.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {meeting.participants.map((p, i) => {
                      ...
                      return (
                        <div key={i} style={{ background: '#111110', border: '1px solid #252523', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: '#e8e6df' }}>
                              {p.name && p.name !== p.email ? p.name : p.email}
                            </div>
                            {p.name && p.name !== p.email && (
                              <div style={{ fontSize: 11, color: '#4a4845', marginTop: 2 }}>{p.email}</div>
                            )}
                          </div>
                          {linkedLead && (
                  ...
                  <div style={{ textAlign: 'center', padding: '40px 20px', color: '#4a4845' }}>
            ...
            {activeTab === 'notes' && (
              <div>
                <div style={{ fontSize: 11, color: '#4a4845', marginBottom: 10 }}>
                ...
                <textarea
                  ...
                  style={{
                    ...
                    background: '#111328', border: '1px solid #252523', borderRadius: 8,
        ...
        <div style={{
          ...
          {/* Header */}
          <div style={{ padding: '14px 16px', borderBottom: '1px solid #252523', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <MessageSquare size={15} color="#784bd1" />
              <span style={{ fontSize: 14, fontWeight: 700, color: '#e8e6df' }}>Meeting Sidekick</span>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {chat.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '28px 10px', color: '#4a4845' }}>
                ...
                <div style={{ fontSize: 11, color: '#4a4845', lineHeight: 1.65 }}>
                ...
                <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {[
                    ...
                    <button
                      ...
                      style={{
                        background: '#111110', border: '1px solid #252523', borderRadius: 7,
              ...
              chat.map((msg, i) => (
                <div key={msg.id || i} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  ...
                  <div style={{ fontSize: 10, color: '#4a4845', marginTop: 2 }}>
            ...
            {chatLoading && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: '#4a4845', fontSize: 12 }}>
          ...
          {/* Input */}
          <form onSubmit={handleSendChat} style={{ padding: '10px 12px', borderTop: '1px solid #252523', flexShrink: 0 }}>
      ...
      {showLinkModal && (
        ...
        >
          <div style={{ background: '#161614', border: '1px solid #252523', borderRadius: 12, width: 440, maxHeight: '70vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #252523', flexShrink: 0 }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {filteredLeads.length === 0 ? (
                <div style={{ textAlign: 'center', padding: 30, color: '#4a4845', fontSize: 13 }}>No leads found</div>
                ...
                return (
                  <div
                    ...
                    style={{ padding: '11px 16px', borderBottom: '1px solid #111110', cursor: alreadyLinked ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'background 0.1s', opacity: alreadyLinked ? 0.6 : 1 }}
                    onMouseEnter={e => { if (!alreadyLinked) e.currentTarget.style.background = '#111110'; }}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                  >
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#e8e6df' }}>{lead.name || '(no name)'}</div>
