...
import { getUpcomingMeetings, getPastMeetings, getImsgThreads, getImsgDirectory, getClients, getTimeEntries, getRoutines, getTeamTodos } from '../lib/api';
import { C, T, F } from '../lib/theme';
import { Screen, Tile, Label, Avatar, Button, Dot, Progress, DOCK_SPACE } from '../components/ui';
import { setInboxUnread, unreadOf } from '../lib/inboxBadge';
import { last10 } from '../lib/imsg';
import { todayProgress } from '../lib/tasks';

// Home: a grid of live tiles. Each opens a space. The numbers come from the
// same endpoints the spaces use, loaded together and refreshed on focus.
const pad = (n) => String(n).padStart(2, '0');
const dstr = (d) => `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
const fmtTime = (iso) => new Date(iso).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
const fmtHM = (min) => { const m = Math.max(0, Math.round(min)); const h = Math.floor(m / 60); return h ? `${h}h ${m % 60}m` : `${m}m`; };
...
export default function HomeScreen({ navigation }) {
  ...
  const load = useCallback(async (quiet) => {
    ...
    const [up, past, th, dir, cl, tm, rt, td] = await Promise.allSettled([
      getUpcomingMeetings(), getPastMeetings(), getImsgThreads(), getImsgDirectory(), getClients(), getTimeEntries(me?.id), getRoutines(), getTeamTodos(),
    ]);
    ...
    setTasks(todayProgress(ok(rt), ok(td), me?.id));
    setNow(Date.now());
    setRefreshing(false);
  }, [me?.id]);
  ...
  const needs = unreadThreads.length + hot + (tasks.total - tasks.done > 0 ? 1 : 0);
    ...
    <Screen aurora>
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 16, paddingHorizontal: 18, paddingBottom: DOCK_SPACE, gap: 14 }}
        ...
        <View style={{ paddingHorizontal: 4, gap: 4 }}>
          <Text style={T.h1}>{greeting()}{me ? `, ${cap(firstName(me))}` : ''}</Text>
          <Text style={T.sub}>{dateLine} · {needs === 0 ? 'nothing waiting on you' : `${needs} thing${needs === 1 ? '' : 's'} need${needs === 1 ? 's' : ''} you`}</Text>
        </View>
        ...
        <Tile white onPress={() => go('Calendar')} style={{ flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: 'rgba(255,255,255,0.78)' }}>
          ...
          {nextUp?.meet_link ? <Button label="Join" small onPress={() => Linking.openURL(nextUp.meet_link)} /> : nextUp?.location ? (
            <Button label="Directions" small kind="soft" onPress={() => Linking.openURL(`https://maps.apple.com/?q=${encodeURIComponent(nextUp.location)}`)} />
          ) : null}
        </Tile>
