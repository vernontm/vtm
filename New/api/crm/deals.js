...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      if (id) {
        const [item] = await supaFetch(`crm_deals?id=eq.${id}${scopeFilter}`);
        return item ? res.json(item) : res.status(404).json({ error: 'Not found' });
      }
      const filter = all ? '' : `client_id=eq.${clientId}&`;
      return res.json(await supaFetch(`crm_deals?${filter}order=created_at.desc`));
    }
    ...
    if (req.method === 'POST') {
      ...
      const result = await supaFetch('crm_deals', { method: 'POST', body: JSON.stringify(payload) });
    ...
    if (req.method === 'PUT' && id) {
      ...
      const result = await supaFetch(`crm_deals?id=eq.${id}${scopeFilter}`, { method: 'PATCH', body: JSON.stringify(data) });
    ...
    if (req.method === 'DELETE' && id) {
      await supaFetch(`crm_deals?id=eq.${id}${scopeFilter}`, { method: 'DELETE' });
